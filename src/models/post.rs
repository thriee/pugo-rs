...
impl Post {
    /// Create a new Post with sensible defaults (comments enabled).
    pub fn new() -> Post {
        Post {
            meta: PostMetadata {
                comments: Some(true),
    ...
    pub fn from_str(content: &str) -> Result<Post, Box<dyn std::error::Error>> {
        ...
        // Determine format from the first line
        ...
        if first_line == "```toml" {
        ...
        } else if first_line == "```yaml" {
            // YAML fenced metadata: ```yaml ... ```
            for line in lines.by_ref() {
                if line == "```" {
                    break;
                }
            ...
            // YAML without opening --- (generated by to_file): <YAML> ... ---
            metadata_string.push_str(first_line);
            metadata_string.push('\n');
            for line in lines.by_ref() {
                if line == "---" {
                    break;
        ...
        // Remaining lines are the markdown content
        for line in lines {
        ...
        if metadata_format == "yaml" {
            post.meta = serde_yaml_ng::from_str(metadata_string.as_str())?;
    ...
    pub fn list_from_dir(dir_path: &str) -> Result<Vec<Post>, Box<dyn std::error::Error>> {
        ...
        // sort by date
        posts.sort_by_key(|b| std::cmp::Reverse(b.datetime.unwrap()));
        Ok(posts)
    }

    pub fn to_file(&self, file_path: &str) -> Result<(), Box<dyn std::error::Error>> {
        let mut post_string = String::from("");
        let metadata_string = serde_yaml_ng::to_string(&self.meta).unwrap();
...
Workers Sites leverages the power of Workers and Workers KV by allowing developers to upload their sites directly to the edge, and closer to the end users. <!--more--> Born on the edge, Workers Sites is what we think modern development on the web should look like, natively secure, fast, and massively scalable. Less of your time is spent on configuration, and more of your time is spent on your code, and content itself."
    ...
    #[test]
    fn test_parse_post_no_opening_delimiter() {
        // Format generated by Post::to_file() — YAML without opening ---
        let post_content = "title: Test No Opening Delimiter
author: tester
date: 2023-01-15 10:30:00
tags:
- rust
- test
slug: no-opening-test
---
This is the content without an opening delimiter. <!-- more --> More content here.";
        let post = Post::from_str(post_content).unwrap();
        assert_eq!(post.meta.title, "Test No Opening Delimiter");
        assert_eq!(post.meta.slug, "no-opening-test");
        assert_eq!(post.meta.date, "2023-01-15 10:30:00");
        assert_eq!(post.meta.author, Some("tester".to_string()));
        assert_eq!(
            post.meta.tags.as_ref().unwrap().join(","),
            "rust,test"
        );
        assert_eq!(post.brief_markdown, "This is the content without an opening delimiter.");
        assert!(post.datetime.unwrap().year() == 2023);
    }
